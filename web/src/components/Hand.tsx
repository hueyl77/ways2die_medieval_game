...
export function Hand({ view, cards, selected, assignments, onSelect }:
  { view: PlayerView; cards: CardView[]; selected: string | null; assignments: Record<number, string>; onSelect: (id: string) => void }) {
  ...
  return (
    <div className="flex gap-2 overflow-x-auto scrollbar-thin py-2 px-1">
      {sorted.map((c) => { const tag = assignedTo(c.id); return (
        <div key={c.id} className="shrink-0"><CardFace cardKey={c.key} width={84} selected={selected === c.id} dimmed={!!tag && selected !== c.id} tag={tag ? `→ ${tag}` : undefined} onClick={() => onSelect(c.id)} /></div>
      ); })}
