...
const Portrait = ({ trade, width = 150 }: { trade: (typeof TRADES)[number]; width?: number }) => (
  <div className="card-face relative rounded-md overflow-hidden shadow-card bg-night-3 shrink-0" style={{ width }}>
...
export default function Rules() {
  const nav = [['idea', 'The idea'], ['envelope', 'Your envelope'], ['round', 'A round'], ['resolve', 'How a pile resolves'], ['wounds', 'Wounds & gold'], ['trades', 'The eight trades'], ['death', 'Death & ghosts'], ['winning', 'Winning'], ['manners', 'Table manners'], ['seats', 'Seats & rounds']];
  return (
    <div className="min-h-full">
      <header className="bg-gradient-to-b from-night-2 to-night px-6 pt-10 pb-8 border-b border-night-3">
        <div className="max-w-5xl mx-auto flex flex-wrap items-end gap-8">
          <div className="flex-1 min-w-[260px]">
            ...
            <p className="mt-4 text-base max-w-2xl text-parchment/90">A Million Ways to Die in Medieval is a social deduction game in the werewolf family, with one twist: nobody at the table gets to be the innocent villager. Everyone has a secret role, and everyone is a werewolf (a killer). Each of you runs a hidden trade, each of you is quietly arranging accidents for the neighbors, and all of it happens through cards placed face-down. There is no night phase, no moderator, and no part where everyone closes their eyes and hums while one player points around the table. The lying, accusing, and deducing all happen out loud, in the open, while the cards keep everyone's secrets.</p>
      ...
      <main className="max-w-5xl mx-auto px-6 pb-20">
        <Section id="idea" eyebrow="The idea" title="Get rich, stay alive.">
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-night-2 border border-night-3 rounded-md p-4 flex gap-3"><Portrait trade="thief" width={90} /><div><div className="font-display text-gold text-lg">A secret trade</div><p className="text-sm">At the start of the game you open an envelope that has a full deck of your role's abilities. Nobody else knows which one you drew.</p></div></div>
            <div className="bg-night-2 border border-night-3 rounded-md p-4 flex gap-3"><CardFace cardKey="job:jeweler" width={90} preview={false} /><div><div className="font-display text-gold text-lg">Gold for your trade</div><p className="text-sm">Gold is earned by <em>trades</em>, not by players, and it's tracked in the open on a public board. Every time you sell your wares your role's track fills up a little more, and tells the table a little more about who you might be.</p></div></div>
            <div className="bg-night-2 border border-night-3 rounded-md p-4 flex gap-3"><CardFace cardKey="mishap:bee-swarm" width={90} preview={false} /><div><div className="font-display text-gold text-lg">Accidents happen</div><p className="text-sm">Every card you play lands face-down and gets shuffled before anyone sees it. Nobody can ever prove who arranged the goose.</p></div></div>
          </div>
        ...
        <Section id="envelope" eyebrow="Your envelope" title="What you hold">
          <p className="max-w-2xl mb-4">Every seat holds the same shape of hand: enough cards to place one in front of every seat, every round.</p>
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="text-center"><CardFace cardKey="alms:miller" width={104} /><div className="text-xs mt-1 font-ui">Alms × 1 · printed for your trade</div></div>
            ...
            <div className="text-center"><CardFace cardKey="mishap:hidden-rake" width={104} /><div className="text-xs mt-1 font-ui">Mishaps × 3 · 1 wound each</div></div>
            ...
            <div className="text-center"><CardFace cardKey="sig:hearty-stew" width={104} /><div className="text-xs mt-1 font-ui">Signature × 3 · your trade's own</div></div>
          </div>
          <div className="font-display text-gold text-lg mt-6">How the cards behave</div>
          <ul className="mt-2 space-y-1.5 max-w-2xl text-sm">
            ...
            <li><b className="text-gold">Alms</b> is judged first, before the round's wares are counted: if your trade is clearly last or second-to-last among the trades still in play (ties, like everyone at 0, do nothing), it gains 4 gold. Playing it also tells the table your trade is in the game.</li>
            <li><b className="text-gold">Tax Collector</b> shuts down a pile's takings: every coin that would be earned from the cards in that pile this round (wares, Alms, gold-gaining signature cards) goes to the crown instead. Drop it where you expect a rival's wares to land.</li>
            <li><b className="text-gold">Signature cards</b> are unique to your trade: powerful, and a confession. Revealing one proves your trade is at the table (though never which chair it's sitting in).</li>
          </ul>
        </Section>
        ...
        <Section id="round" eyebrow="A round" title="Place, lock in, reveal">
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <Step n={3} title="Shuffle and reveal" art={<img src="/cards/basic-card-back.jpg" alt="card back" className="rounded-md shadow-card" style={{ width: 72 }} />}>Each pile is shuffled before it flips, so no card can ever be traced back to the person who placed it. The reveal plays one scene at a time (each pile, then wounds, deaths, and the ledger) and moves on when everyone clicks Next.</Step>
          </div>
        ...
        <Section id="resolve" eyebrow="How a pile resolves" title="In this order, every pile">
          <ol className="grid md:grid-cols-2 gap-2 list-none max-w-3xl">
            {[
              ...
              ['Deaths', 'Reach your death number and you die at the end of the pile. The funeral comes after the round.'],
              ['Gold', 'Alms is judged first on the board as it stood; then wares bank 1 each; then signature gold effects.'],
              ['Words', 'Truth cards (Inquest, Appraisal) are answered by the game itself, so no lying is possible.'],
        ...
        <Section id="wounds" eyebrow="Wounds & gold" title="Every wound is a coin">
          <div className="grid md:grid-cols-2 gap-6 items-start">
            <div>
              <p className="text-sm">Wounds are the attack cards themselves, left face-up in front of you, so everyone can see how close to death everyone else is. Hit your death number (<b className="text-gold">3 wounds at a table of 4 or 5, 4 wounds at 6 or more</b>) and you die.</p>
        ...
        <Section id="seats" eyebrow="Seats & rounds" title="How long is a year?">
          <p className="max-w-2xl text-sm mb-3">The host sets the table size (4 to 8). Seats without a player are filled by bots, who play random cards and can win if they out-earn you.</p>
